...
df6_weekly_counts <- get_weekly_counts(df6, "Initial_Intake_Date")
...
temp <- all_weekly_counts |>
  filter(source_file == "From-Emily-FOIA-10-2554-527" | source_file == "2024-ICFO-41855"|source_file == "From-Emily-Excel-X-RIF"|source_file == "uwchr"|source_file == "120125")

ggplot(temp ,aes(week_start, y=n, color = source_file))+
  geom_line(alpha = 0.5)+
  theme_minimal()

# at minimum, we merge "120125", "2024-ICFO-41855", "From-Emily-Excel-X-RIF", "From-Emily-FOIA-10-2554-527", and "uwchr"  
rm(df1, df2, df8) # remove these to clear memory space
gc()
