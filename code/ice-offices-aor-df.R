...
hq_aor_df <- tibble(`Area of Responsibility` = "HQ Area of Responsibility")

aor_sf <-
  bind_rows(aor_sf, hq_aor_df)
...
arrests |>
  ...
  left_join(
    aor_sf |>
      ...
      mutate(in_aor_sf = 1),
    by = c("apprehension_aor" = "Area of Responsibility")
  ) |>
  confront(rules) |>
  summary() |>
  (\(x) {
